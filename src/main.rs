...
async fn main() {
    ...
    let port: u16 = dotenv::var("PATH_OVERRIDE")
        .map(|s| {
            s.parse::<u16>()
                .expect("PATH_OVERRIDE in .env file is not a number")
        })
        .unwrap_or(7272);
    let initial_path: String = dotenv::var("INITIAL_PATH").unwrap_or("index.html".to_string());
    ...
    let server = warp::serve(files.or(env)).run(([0, 0, 0, 0], port));
